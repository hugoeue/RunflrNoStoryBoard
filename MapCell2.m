...
{
    MapCell2 *aView = nil;
    NSArray *objs = [[NSBundle mainBundle] loadNibNamed:@"MapCell2" owner:nil options:nil];
    for(id obj in objs) {
        if([obj tag]==100 ){
...
-(void)toAppearItemsView
{
  ...
  /*  self.hotelName.text=self.placeDetailVO.pNameStr;
...
- (IBAction)clickButton:(UIButton *)sender {
  /*  ButtonType2 aType = ButtonType_Profile2;
    NSLog(@"button Clicked %ld", (long)sender.tag);
    switch (sender.tag) {
        case 100:
            NSLog(@"Click button1");
            aType = ButtonType_Profile2;
            break;
        case 101:
            NSLog(@"Click button2");
            aType = ButtonType_Distance2;
            break;
        case 102:
            NSLog(@"Click button3");
            aType = ButtonType_More2;
            break;
