             ...
             ignoreCase:(BOOL)ignoreCase
{
    ...
    else if (textField.autocompleteType == HTAutocompleteTypeColor)
    {
        ...
        {
            NSMutableArray * merdas = [NSMutableArray new];
             for (City * vitie in [Globals cities]) {
                 //NSLog(@"Citie name: %@ ",vitie.name);
                 [merdas addObject:vitie.name];
             }
            
            colorAutocompleteArray = merdas;
    ...
    }else if (textField.autocompleteType == HTAutocompleteTypeRest)
    {
        static dispatch_once_t colorOnceToken;
        static NSArray *colorAutocompleteArray;
        dispatch_once(&colorOnceToken, ^
                      {
                          NSMutableArray * merdas = [NSMutableArray new];
                          for (Restaurant * vitie in [Globals searchResult]) {
                              //NSLog(@"Citie name: %@ ",vitie.name);
                              [merdas addObject:vitie.name];
                          }
